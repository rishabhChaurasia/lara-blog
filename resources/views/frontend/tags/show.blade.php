...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="text-center mb-12">
        <h1 class="text-3xl font-extrabold text-gray-900 sm:text-4xl">#{{ $tag->name }}</h1>
        <p class="mt-3 max-w-2xl mx-auto text-xl text-gray-500 sm:mt-4">
            Posts tagged with {{ $tag->name }}
        </p>
        <p class="mt-2 text-gray-600">{{ $posts->total() }} posts with this tag</p>
    </div>
    ...
    <div class="flex flex-col md:flex-row gap-8">
        <div class="md:w-3/4">
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="mt-1 text-gray-500">There are no published posts with this tag yet.</p>
                </div>
            @endif
        </div>
        ...
        <div class="md:w-1/4">
            <div class="bg-white shadow rounded-lg p-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4">All Tags</h3>
                <div class="flex flex-wrap gap-2">
                    @foreach($tags as $t)
                    <a href="{{ route('tags.show', $t->slug) }}" 
                       class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $t->id === $tag->id ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-800 hover:bg-gray-200' }}">
                        {{ $t->name }}
            ...
            <div class="mt-6 bg-white shadow rounded-lg p-6">
                ...
                <ul class="space-y-2">
                    @forelse($posts->pluck('categories')->flatten()->unique('id')->sortByDesc('posts_count')->take(5) as $category)
                    <li>
                        <a href="{{ route('categories.show', $category->slug) }}" 
